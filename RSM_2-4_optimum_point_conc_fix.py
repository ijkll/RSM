...
model_expr = (
    ...
    + reduced_model.params['X1'] * X1_fixed
    + reduced_model.params['X2'] * X[0]
    + reduced_model.params['X3'] * X[1]
    + reduced_model.params['I(X1 ** 2)'] * X1_fixed**2
    + reduced_model.params['I(X2 ** 2)'] * X[0]**2
    + reduced_model.params['I(X3 ** 2)'] * X[1]**2
    + reduced_model.params['X1:X2'] * X1_fixed * X[0]
    + reduced_model.params['X1:X3'] * X1_fixed * X[1]
    + reduced_model.params['X2:X3'] * X[0] * X[1]
)
...
# 경계값 설정 (X1, X2만 최적화)
lbx = [X2_min, X3_min]  # 하한
ubx = [X2_max, X3_max]  # 상한
...
# 결과 출력
print(f"🔹 X3 = {X1_fixed}로 고정한 상태에서 최적화 결과")
print("----------------------------------------------------")
print(f"   최적화된 Y 값 (PCE) = {optimal_Y:.4f}")
print(f"   최적 X1 = ({X1_fixed} 고정), X2 = {optimal_X[0]:.4f}, (X3 = {optimal_X[1]:.4f}")
print("----------------------------------------------------")
